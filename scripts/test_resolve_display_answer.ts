/**
 * Run: cd frontend && npx --yes tsx ../scripts/test_resolve_display_answer.ts
 */
import { mergeDisplayAnswer, buildFallbackFinalAnswer } from '../frontend/src/utils/parseProcessLog'
...
assert(
  mergeDisplayAnswer(modelShort, pocketTable).includes('| EGFR_3W2S_pocket1 |'),
  'prefer pocket table over short model summary',
)

const fromSteps = buildFallbackFinalAnswer([
  {
    type: '工具',
    title: '口袋预测',
    name: '口袋预测',
